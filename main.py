...
class App:
    ...
    def proceed_command(self):
        ...
        for diff_object in added_files + modified_files + renamed_files + deleted_files:
            if diff_object.a_blob:
                final_path = path.abspath(path.join(original_files, diff_object.a_path))
                os.makedirs(path.dirname(final_path), exist_ok=True)
                diff_object.a_blob.stream_data(open(final_path, "wb"))
            if diff_object.b_blob:
                final_path = path.abspath(path.join(updated_files, diff_object.a_path))
                os.makedirs(path.dirname(final_path), exist_ok=True)
